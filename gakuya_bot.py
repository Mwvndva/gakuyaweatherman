...
# Validate API keys
if not all([GROK_API_KEY, OPENWEATHER_API_KEY, X_API_KEY, X_API_SECRET, X_ACCESS_TOKEN, X_ACCESS_TOKEN_SECRET]):
...
# Function to get trending topics in Kenya
def get_trending_topics():
    try:
        # Kenya's WOEID (Where On Earth ID)
        kenya_woeid = 23424863
        trends = api.get_place_trends(kenya_woeid)
        if trends and len(trends) > 0:
            # Get top 3 trending topics
            top_trends = trends[0]['trends'][:3]
            return [trend['name'] for trend in top_trends]
        return []
    except Exception as e:
        logger.error(f"Failed to fetch trending topics: {e}")
        return []

# Function to generate weather post with trending topics
def generate_weather_post(temp, description):
    trending_topics = get_trending_topics()
    trending_hashtags = " ".join(trending_topics) if trending_topics else ""
    
    prompt = f"""
    You are Gakuya, a Kenyan AI bot with a funny, sarcastic, and unhinged personality. Create a short X post (130 characters or less) in English that combines today's weather in {CITY} (Temp: {temp}°C, Condition: {description}) with current trending topics in Kenya: {trending_hashtags}. Use Kenyan banter with relatable references (e.g., matatus, tea, maandazi, or local vibes), and humor. Include relevant hashtags from the trending topics and #NairobiWeather #KenyanVibes. Keep it short, punchy, and chaotic. Avoid Sheng slang.
    """
    try:
